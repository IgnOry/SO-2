 ...
 *    The filler function will not return '1' (unless an error happens), so the whole directory is readBuf in a single readdir operation.
 *
...
{
	int i = 0;	//for the fors (saw it used on C examples and found it cleaner)
	char* filename = (char*)(path + 1);	//for findFileByName I need the path WITHOUT the "/" before it

	//does file exist?

	int nodeID = findFileByName(&myFileSystem, filename);
	...
	//file exists!
	//clean directory, update filesystem
	...
	//save the file's i-node to access node array

	int iNodeID = myFileSystem.directory.files[nodeID].nodeIdx; 
	NodeStruct* iNode = myFileSystem.nodes[iNodeID];

	//set all blocks to free, update bitmap and superblock

	for (i = 0; i < iNode->numBlocks; i++)
		myFileSystem.bitMap[iNode->blocks[i]] = 0;

	myFileSystem.superBlock.numOfFreeBlocks += iNode->numBlocks;
	updateBitmap(&myFileSystem);
	updateSuperBlock(&myFileSystem);

	//clean up the node and update it

	iNode->numBlocks = 0;
	iNode->fileSize = 0;
	iNode->freeNode = true;

	updateNode(&myFileSystem, iNodeID, iNode);
 ...
 * @return 0 on success and <0 on error
 **/
static int my_read(const char* path, char* readBuf, size_t size, off_t offset, struct fuse_file_info* info)
{
	char buffer[BLOCK_SIZE_BYTES];
	int totalBytes = size, totalRead = 0;
	NodeStruct* node = myFileSystem.nodes[info->fh];


	fprintf(stderr, "--->>>my_read: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, info->fh);
	...
	while (totalBytes) {
		...
		// move to current offset, then read from current offset
		// if it fails, return -EIO and print which failed
		if (lseek(myFileSystem.fdVirtualDisk, currentBlock * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t)-1)
		{
			perror("Failed lseek in my_write");
			return -EIO;
		}
		else if((readReturn = read(myFileSystem.fdVirtualDisk, &buffer, BLOCK_SIZE_BYTES)) == -1)
		{
			perror("Failed read in my_write");
			return -EIO;
		}
			...
			for (i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalRead < size); i++) {
				readBuf[totalRead] = buffer[i];
				totalRead++;
			}
			totalBytes -= (i - offBlock);
			offset += i;
		}
	}

	//make sure everything's modified, saved and written
	sync();
...
struct fuse_operations myFS_operations = {
	...
	.unlink		= my_unlink,					// unlink a file
