...
static int my_unlink(const char *path) //Orden copiado del pseudocodigo dado en diapos, igual hay que cambiar cosas
{
    ...
    int idxNode = findFileByName(&myFileSystem, filename); 
	...
	updateNode(&myFileSystem, auxIdNode, iNode);

    //Liberar la memoria del nodo-i y actualizar la tabla
    int nBlocks = iNode->numBlocks;
    ...
    myFileSystem.superBlock.numOfFreeBlocks += iNode->numBlocks; //Actualizar valor
	updateBitmap(&myFileSystem); //Actualizar tabla
	updateSuperBlock(&myFileSystem); //Actualizar tabla

	iNode->numBlocks = 0;
	iNode->fileSize = 0;

	fprintf(stderr, "Archivo borrado\n");
